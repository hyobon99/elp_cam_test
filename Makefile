# OpenCV 웹캠 뷰어 Makefile
# 지원 플랫폼: Linux, macOS, Raspberry Pi (ARM)

# 플랫폼 감지
UNAME_S := $(shell uname -s)
UNAME_M := $(shell uname -m)

# 기본 설정
CXX = g++
CXXFLAGS = -g -std=c++11 -Wall -Wextra
LDFLAGS = 

# OpenCV 설정 (플랫폼별)
ifeq ($(UNAME_S),Darwin)
    # macOS
    PKG_OPENCV_CFLAGS := $(shell pkg-config --cflags opencv4 2>/dev/null || pkg-config --cflags opencv 2>/dev/null)
    PKG_OPENCV_LIBS := $(shell pkg-config --libs opencv4 2>/dev/null || pkg-config --libs opencv 2>/dev/null)
    PLATFORM = macos
    CXXFLAGS += -DMACOS
else ifeq ($(UNAME_S),Linux)
    ifeq ($(UNAME_M),aarch64)
        # Raspberry Pi 4 (ARM64)
        PKG_OPENCV_CFLAGS := $(shell pkg-config --cflags opencv4 2>/dev/null || pkg-config --cflags opencv 2>/dev/null)
        PKG_OPENCV_LIBS := $(shell pkg-config --libs opencv4 2>/dev/null || pkg-config --libs opencv 2>/dev/null)
        PLATFORM = raspberry_pi
        CXXFLAGS += -DRASPBERRY_PI -march=native
    else ifeq ($(UNAME_M),armv7l)
        # Raspberry Pi 3 (ARM32)
        PKG_OPENCV_CFLAGS := $(shell pkg-config --cflags opencv4 2>/dev/null || pkg-config --cflags opencv 2>/dev/null)
        PKG_OPENCV_LIBS := $(shell pkg-config --libs opencv4 2>/dev/null || pkg-config --libs opencv 2>/dev/null)
        PLATFORM = raspberry_pi
        CXXFLAGS += -DRASPBERRY_PI -march=native
    else
        # 일반 Linux (x86_64)
        PKG_OPENCV_CFLAGS := $(shell pkg-config --cflags opencv4 2>/dev/null || pkg-config --cflags opencv 2>/dev/null)
        PKG_OPENCV_LIBS := $(shell pkg-config --libs opencv4 2>/dev/null || pkg-config --libs opencv 2>/dev/null)
        PLATFORM = linux
        CXXFLAGS += -DLINUX
    endif
else
    # 기타 플랫폼
    PKG_OPENCV_CFLAGS := $(shell pkg-config --cflags opencv4 2>/dev/null || pkg-config --cflags opencv 2>/dev/null)
    PKG_OPENCV_LIBS := $(shell pkg-config --libs opencv4 2>/dev/null || pkg-config --libs opencv 2>/dev/null)
    PLATFORM = unknown
endif

# OpenCV 라이브러리 설정
CXXFLAGS += $(PKG_OPENCV_CFLAGS)
LDFLAGS += $(PKG_OPENCV_LIBS)

# 타겟들
TARGET_SDK = webcam_viewer_sdk
TARGET_REF = reference_viewer
SOURCE_SDK = webcam_viewer_sdk.cpp
SOURCE_REF = reference.cpp

# 기본 타겟
all: $(TARGET_SDK) $(TARGET_REF)

# SDK 뷰어 빌드
$(TARGET_SDK): $(SOURCE_SDK)
	@echo "빌드 중: $@ (플랫폼: $(PLATFORM))"
	$(CXX) $(CXXFLAGS) $< -o $@ $(LDFLAGS)

# Reference 뷰어 빌드
$(TARGET_REF): $(SOURCE_REF)
	@echo "빌드 중: $@ (플랫폼: $(PLATFORM))"
	$(CXX) $(CXXFLAGS) $< -o $@ $(LDFLAGS)

# 플랫폼 정보 출력
info:
	@echo "=== 플랫폼 정보 ==="
	@echo "OS: $(UNAME_S)"
	@echo "Architecture: $(UNAME_M)"
	@echo "Platform: $(PLATFORM)"
	@echo "C++ Compiler: $(CXX)"
	@echo "OpenCV CFLAGS: $(PKG_OPENCV_CFLAGS)"
	@echo "OpenCV LIBS: $(PKG_OPENCV_LIBS)"

# OpenCV 설치 확인
check-opencv:
	@echo "=== OpenCV 설치 확인 ==="
	@pkg-config --version opencv4 2>/dev/null && echo "OpenCV4 발견" || echo "OpenCV4 없음"
	@pkg-config --version opencv 2>/dev/null && echo "OpenCV 발견" || echo "OpenCV 없음"
	@echo "OpenCV 라이브러리 경로:"
	@pkg-config --libs opencv4 2>/dev/null || pkg-config --libs opencv 2>/dev/null || echo "OpenCV 라이브러리를 찾을 수 없습니다"

# 테스트 실행
test: $(TARGET_SDK) $(TARGET_REF)
	@echo "=== 테스트 실행 ==="
	@echo "SDK 뷰어 테스트:"
	@./$(TARGET_SDK) --help 2>/dev/null || echo "SDK 뷰어 실행 실패"
	@echo "Reference 뷰어 테스트:"
	@./$(TARGET_REF) --help 2>/dev/null || echo "Reference 뷰어 실행 실패"

# 정리
clean:
	@echo "빌드 파일 정리 중..."
	rm -f $(TARGET_SDK) $(TARGET_REF)
	rm -f *.o
	rm -f *.exe

# 설치 (선택적)
install: $(TARGET_SDK) $(TARGET_REF)
	@echo "시스템에 설치 중..."
	sudo cp $(TARGET_SDK) /usr/local/bin/
	sudo cp $(TARGET_REF) /usr/local/bin/
	@echo "설치 완료! 다음 명령어로 실행 가능:"
	@echo "  webcam_viewer_sdk"
	@echo "  reference_viewer"

# 제거
uninstall:
	@echo "시스템에서 제거 중..."
	sudo rm -f /usr/local/bin/$(TARGET_SDK)
	sudo rm -f /usr/local/bin/$(TARGET_REF)
	@echo "제거 완료!"

# 도움말
help:
	@echo "=== OpenCV 웹캠 뷰어들 ==="
	@echo "make all          - 모든 뷰어 빌드 (SDK + Reference)"
	@echo "make info         - 플랫폼 정보 출력"
	@echo "make check-opencv - OpenCV 설치 확인"
	@echo "make test         - 빌드된 프로그램 테스트"
	@echo "make clean        - 빌드 파일 정리"
	@echo "make install      - 시스템에 설치"
	@echo "make uninstall    - 시스템에서 제거"
	@echo ""
	@echo "=== Reference 뷰어 (reference_viewer) ==="
	@echo "• 코드 상에서 해상도/FPS 설정 가능"
	@echo "• FPS 제어 (프레임 타이밍 기반)"
	@echo "• 실시간 FPS 모니터링"
	@echo "• 프레임 저장 (s 키)"
	@echo "• 사용법: ./reference_viewer [device] [width] [height] [fps]"
	@echo ""
	@echo "=== SDK 뷰어 (webcam_viewer_sdk) ==="
	@echo "• 해상도 변경 (1-6 키)"
	@echo "• FPS 제어 (F1-F4 키)"
	@echo "• 키프레임 레이트 설정 (k1-k5 키)"
	@echo "• 카메라 정보 표시 (i 키)"
	@echo "• 지원 포맷 목록 (f 키)"
	@echo "• 실시간 FPS 모니터링"
	@echo "• 프레임 저장 (s 키)"

.PHONY: all info check-opencv test clean install uninstall help 